...
{
    public partial class EntriesPerMethod : UserControl
    {
        List<Ip> Ips = new List<Ip>();
        private SimpleError SE;

        public EntriesPerMethod()
        {
            InitializeComponent();
        }
        ...
        public Errors InitializeFilter(Filter filter)
        {
            ...
            if (!ActivateIpCB.Checked && !ActivateTimespanCB.Checked && !ActivateMethodCB.Checked)
            {
            ...
            if (ActivateMethodCB.Checked)
            {
                filter.FilterMethods = true;
                if (GetCB.Checked) filter.Methods.Add(Method.Get);
                if (PostCB.Checked) filter.Methods.Add(Method.Post);
                if (HeadCB.Checked) filter.Methods.Add(Method.Head);
            }
